...
export default function GamificationView({ user, onRefreshStats, onUpdateUserLocal }: GamificationViewProps) {
  ...
  const fetchGamificationData = async () => {
    ...
    try {
      ...
      setChallenges(await readJsonArray(chalRes));
      setBadges(await readJsonArray(badgesRes));
      setRewards(await readJsonArray(rewardsRes));
      setLeaderboard(await readJsonArray(leaderRes));
      setUnlockedBadges(await readJsonArray(userBadgesRes));
      setRedemptions(await readJsonArray(redRes));
      ...
      if (dbParticipationsResponse.ok) {
        const parts = await readJsonArray(dbParticipationsResponse) as ChallengeParticipation[];
        const userParts = parts.filter((p: ChallengeParticipation) => p.userId === user.id);
        setJoinedChallenges(userParts);

        // Populate slider progress states
        const initialProgresses: { [key: string]: number } = {};
        userParts.forEach((p: ChallengeParticipation) => {
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs text-slate-900 font-medium">
                ...
                {leaderboard.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-400 text-xs font-bold uppercase tracking-wider">No leaderboard data available</td>
                  </tr>
                )}
              </tbody>
            </table>
